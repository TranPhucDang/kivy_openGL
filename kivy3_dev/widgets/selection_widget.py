...
from kivy3_dev import Renderer
...
class SelectionWidget(RelativeLayout):
    def __init__(self, renderer, **kw):
        ...
        self.last_sel_image = Texture.create(
            self.renderer.fbo.texture.size, colorfmt="rgba", bufferfmt="ubyte"
        )
        ...
        for i in range(200, 256):
            for j in range(256):
                for k in range(256):
                    ...
                    if color_id not in self.object_dict.keys() and color_id != (
                        0,
                        0,
                        0,
                    ):
    ...
    def get_clicked_object(self, touch):
        ...
        color = tuple(
            self.renderer.fbo.get_pixel_color(
                pos[0] - self.parent.pos[0], pos[1] - self.parent.pos[1]
            )[0:3]
        )
        ...
        self.last_sel_image.blit_buffer(
            self.renderer.fbo.pixels,
            colorfmt="rgba",
            bufferfmt="ubyte",
            size=self.renderer.fbo.size,
        )
        # print(color)
